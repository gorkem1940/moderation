...
exports.run = async(client, message, args) => {
   if (!message.member.roles.cache.has("849341898245275688","849341802653024296","849351388857499658") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new Discord.MessageEmbed().addField("Yetersiz Yetki",` Bu komutu kullanabilmen için gerekli yetkin yok.`).setColor("RANDOM")).then(m => m.delete({timeout: 7000}));
   let member = message.mentions.users.first() || client.users.cache.get(args.join(' '))
   if(!member) {
       return message.channel.send('\・Bir kişi etiketlemelisin.')
   }
   let tanrısızvip = message.guild.roles.cache.find(r => r.id === '848127316680638505') // Rol id
